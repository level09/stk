...
This file provides comprehensive architectural patterns and coding standards for AI agents working with the stk framework.

## Framework Overview

**stk** is an async Quart-based web framework optimized for rapid development with modern tooling:
...
The app is created using the factory pattern in `stk/app.py`:
...
Extensions are initialized in `stk/extensions.py`:
...
- Vue 3 and Vuetify loaded from local static files (`stk/static/`)
- Components defined using `Vue.defineComponent` with template strings
- Per-page Vue instances (not SPA architecture)
- Global configuration in `stk/static/js/config.js`

### CRITICAL: Custom Vue Delimiters

**IMPORTANT**: stk uses `${` and `}` for Vue expressions to avoid conflicts with Jinja's `{{ }}`:
...
Use `stk.tasks.run_in_background()` for fire-and-forget async tasks:
...
RUN useradd -m -u 1000 stk && \
    chown -R stk:stk /app /opt/venv
...
services:
  web:
    ...
    environment:
      ...
      - SQLALCHEMY_DATABASE_URI=postgresql://stk:${DB_PASSWORD}@postgres/stk
...
Configuration in `stk/settings.py`:
...
SQLALCHEMY_DATABASE_URI=sqlite:///stk.sqlite3
